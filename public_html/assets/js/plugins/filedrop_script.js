$(function(){
	...
	dropbox.filedrop({
		...
		url: ajaxPath + 'image_uploader.php',
	...
	// custom Images uploads javascript

	$.ajax({
		url : ajaxPath + 'image_loader.php',
		async : true,
		dataType : 'json',
		global : false,
		type : 'post',
		...
		success : function(data){
			if (data[0] !== false) {
				$(data).each(function(key,value){
					$("#uploaded-images").append('<img src="' + assetsPath + 'img/uploads/' + value + '" height="200px" width="200px" class="span2" onmouseover="hover(\''+ value +'\')" onmouseleave="unhover()" id="' + value + '"/>');
				});
			}
	...
	function hover(file)
	{
		...
		filepath = assetsPath + 'img/uploads/' + file.toString();
	...
	{	
		$.ajax({
			...
			success : function (data){
				...
				if(data[0] === true)
				{
					$('img[src="' + assetsPath + 'img/uploads/' + $('.delete-button').attr('alt') + '"]').remove();
